...
def generate_image(prompt):
    ...
    log(f'{prompt}') 
    # Define the API URL
    sd_api_url = 'https://api.gradio.live/sdapi/v1/txt2img' # DEAD - SD (XL) endpoint // public-api
...
def query_model(prompt):
    endpoint = "https://api.trycloudflare.com/v1/completions" # DEAD Text-Generation-WebUI API
...
def toggle_F2():
    ...
    # Implement specific action for button F2 # // NSFW FUNCTIONS DISABLED (See Comment)
    if button_states['F2'] and "(NSFW:1.0), " not in prompt:
        prompt = "(NSFW:1.0), " + prompt
        was_updated = True
    elif not button_states['F2'] and "(NSFW:1.0), " in prompt:
        prompt = prompt.replace("(NSFW:1.0), ", "")
        was_updated = True
...
def toggle_F3():
    ...
    # Implement specific action for button F3 (LORA) APPEND # // NEEDS HELP
    if button_states['F3'] and ",  <lora:lora_filename:0.8>" not in prompt:
        prompt += ",  <lora:lora_filename:0.8>"
        was_updated = True
    elif not button_states['F3'] and ",  <lora:lora_filename:0.8>" in prompt:
        prompt = prompt.replace(",  <lora:lora_filename:0.8>", "")
        was_updated = True
...
prompt = "(White Outline:1), (Black Background:1), <lora:lora_filename:0.8>"
